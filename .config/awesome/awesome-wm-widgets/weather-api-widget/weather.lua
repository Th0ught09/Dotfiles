...
local function get_locale(data)
	...
	local lang = gears.filesystem.file_readable(WIDGET_DIR .. "/" .. "locale/" .. data .. ".lua") and data or "en"

	local locale = require("awesome-wm-widgets.weather-api-widget.locale." .. lang)

	if data ~= lang then
		show_warning(string.format("Your language (%s) is not supported yet. Language set to English", data), locale)
	end
...
local function worker(user_args)
	...
	local current_weather_widget = wibox.widget({
		...
		update = function(self, weather)
			...
			self:get_children_by_id("icon")[1]
				:set_image(ICONS_DIR .. icon_map[weather.condition.code] .. day_night_extension .. icons_extension)
			...
			self:get_children_by_id("feels_like_temp")[1]
				:set_text(locale.feels_like .. gen_temperature_str(weather.feelslike_c, "%.0f", false, units))
			self:get_children_by_id("description")[1]:set_text(weather.condition.text)
			self:get_children_by_id("wind")[1]
				:set_markup(locale.wind .. "<b>" .. weather.wind_kph .. "km/h (" .. weather.wind_dir .. ")</b>")
	...
	weather_widget:buttons(gears.table.join(awful.key({ modkey, "Shift" }, "w", function()
